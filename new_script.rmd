...
colnames_act_females <- colnames(reads)[substr(colnames(reads), 4, 4) == "F" & substr(colnames(reads), 5, 5) == "A"]
...
tmm_factor <- calcNormFactors(matrix_norm)
nor_TMM <- cpm(matrix_norm, normalized.lib.size=tmm_factor)
...
```{r}
colnames_inact_females <- colnames(reads)[substr(colnames(reads), 4, 4) == "F" & substr(colnames(reads), 5, 5) == "C"]
...
*3) active males MA*
```{r}
colnames_act_males <- colnames(reads)[substr(colnames(reads), 4, 4) == "M" & substr(colnames(reads), 5, 5) == "A"]
...
*4) Inactive males MC*
```{r}
colnames_inact_males <- colnames(reads)[substr(colnames(reads), 4, 4) == "M" & substr(colnames(reads), 5, 5) == "C"]
...
*interpretation*
by increasing the filtering parameter from 10 to 1000, we notice that the number of reads decreases, reaches from 14434 to 4000 for the genes which have reads of more than 1000 reads
...
```{r}
library(edgeR)
y <- DGEList(counts = nor_TMM)
# Estimate the dispersion level
y <- estimateDisp(y)
plotBCV(y, xlab= "TMM coefficient")
```
...
for (sample_FA in sample_names_FA) {
  ...
  sample_cols_FA <- grep(sample_FA, colnames(reads_filter1000_FA)) # "grep" finds occurrences of a given pattern in a string vector, and returns the corresponding indices or values.
...
```{r}
fuzz <- mfuzz.plot(mfdata.s,cl=N_cl,mfrow = c(3,4), time.labels = colnames(FA_nor_sum_1000),new.window = F)
...
fuzz <- mfuzz.plot(mfdata.s,cl=N_cl,mfrow = c(3,4), time.labels = colnames(FC_nor_sum_1000),new.window = F)
...
```{r}
library("enrichR")
expr_data <- gene_annotations
cluster_data <- N_cl

#gene_lists <- lapply(unique(cluster_data$membership), function(x) rownames(cluster_data$membership[cluster_data$cluster == 29,]))

gene_list <- rownames(cluster_data$membership[cluster_data$cluster == 40,])


enrichment_results <- lapply(gene_list, function(x) enrichr(x, "GO_Biological_Process_2018"))
#print(enrichment_results)

write.table(enrichment_results, file="enrichR_results.txt", sep="\t", row.names=FALSE)

```

```{r}
library(org.Dm.eg.db)
# Convert Gene/FBgn IDs to symboles
...
dbs <- c("Coexpression_Predicted_GO_Cellular_Component_2018", "Coexpression_Predicted_GO_Molecular_Function_2018",
         "Coexpression_Predicted_GO_Biological_Process_2018")

#as an example, getting data for Cluster 16 using only the core elements

if (websiteLive) {
  enriched <- enrichr(gene_symbols, dbs)
}

if (websiteLive) enriched[["Coexpression_Predicted_GO_Cellular_Component_2018"]]

if (websiteLive) plotEnrich(enriched[[3]], showTerms = 20, numChar = 40, y = "Count", orderBy = "P.value")

#write.table(enriched[[3]], file="enrichR_results.txt", row.names=FALSE)
```
...
# Créer un dossier pour enregistrer les fichiers PNG
dir.create("plots", showWarnings = FALSE)

for (i in 1:num_clusters) {
  gene_list <- as.data.frame(acore(mfdata.s, N_cl, min.acore = 0)[i])$NAME
  gene_symbols <- mapIds(org.Dm.eg.db, keys = gene_list, column = "SYMBOL", keytype = "FLYBASE")
  dbs <- "Coexpression_Predicted_GO_Biological_Process_2018"
  result <- enrichr(gene_symbols, dbs)
  
  # Nom du fichier PNG basé sur le numéro de cluster
  filename <- paste0("plots/plot_cluster_", i, ".png")
  
  # Enregistrer le plot en tant que fichier PNG
  png(filename)
  plotEnrich(result$Coexpression_Predicted_GO_Biological_Process_2018, showTerms = 20, numChar = 70, y = "Count", orderBy = "P.value")
  dev.off()
}
...
results <- list()  # Liste pour stocker les résultats par cluster
...
# Exporter les résultats dans des fichiers CSV ou TXT par cluster
for (i in 1:length(results)) {
  result <- results[[i]]
  
  # Créer un nom de fichier unique pour chaque cluster
  output_file <- paste0("cluster_", i, "_results.txt")
  
  # Extraire les informations requises des résultats et les stocker dans un data frame
  ...
  # Extraire les informations de chaque base de données et les ajouter au data frame
  for (db in dbs) {
    terms <- result[[db]]$Term
    pvalues <- result[[db]]$P.value
    genes <- result[[db]]$Genes
    
    # Fusionner les informations de chaque base de données dans le data frame
    output_data <- rbind(output_data, data.frame(Term = terms, P.value = pvalues, Genes = genes, stringsAsFactors = FALSE))
  }
  
  # Enregistrer le data frame dans un fichier CSV ou TXT
